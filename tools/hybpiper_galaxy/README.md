...
* Making use of the new flag to specify the output location in HybPiper version 2.1.3, currently a lot of files are getting copied
as a workaround.
...
Usage:
```console
sh hybpiper_galaxy.sh -h -v\
-r <RAW_READS_ZIP> \
-o <OUTPUT_LOCATION> \
-t <TARGET_FASTA_FILE> \
-f <TARGET_FILE_FORMAT> \
-e <MAPPING_METHOD> \
-i <INTRONERATE_BOOLEAN> \
-m <HEATMAP_BOOLEAN> \
-x <TIME_OUT_NUM>
```

* **-r** the path to the location of the input ZIP file containing the raw reads.
* **-o** is the path to the location of the zip archive where the files HybPiper generates should go.
* **-t** is the path to the location of the fasta file containing the target sequences for HybPiper to map the reads to.
* **-f** is the sequence format of the readfiles, this can be Amino acid sequences or DNA sequences.
* **-e** specifies which mapping method HybPiper should use to map the reads against the target sequences.
* **-i** is a boolean value that indicates whether HybPiper should run intronerate or not.
* **-m** is a boolean value that indicates whether HybPiper should create a gene_recovery heatmap (CURRENTLY UNIMPLEMENTED, was planned but never got implemented due to time constraints and other priorities).
* **-x** is a numerical value that indicates an integer x, which allows HybPiper to kill jobs that take X percent longer than the average job, to prevent jobs getting stuck. 
...
Usage:
```console
python3 generate_hybpiper_commands.py -h -v\
-r <RAW_READS_ZIP> \
-o <OUTPUT_LOCATION> \
-t <TARGET_FASTA_FILE> \
-f <TARGET_FILE_FORMAT> \
...
Alternatively:
```console
python3 generate_hybpiper_commands.py --help --version \
--readfile <RAW_READS_ZIP> \
--output <OUTPUT_LOCATION> \
--targets <TARGET_FASTA_FILE> \
--target_format <TARGET_FILE_FORMAT> \
--engine <MAPPING_METHOD> \
--intronerate <INTRONERATE_BOOLEAN> \
--heatmap_bool <HEATMAP_BOOLEAN> \
--timeout <TIME_OUT_NUM> \
--namelist <NAMELIST_BOOL>
```

* **-r, --readfile** the path to the location of the input ZIP file containing the raw reads.
* **-o, --output** is the path to the location of the zip archive where the files HybPiper generates should go.
* **-t, --targets** is the path to the location of the fasta file containing the target sequences for HybPiper to map the reads to.
* **-f, --target_format** is the sequence format of the readfiles, this can be Amino acid sequences or DNA sequences.
* **-e, --engine** specifies which mapping method HybPiper should use to map the reads against the target sequences.
* **-i, --intronerate** is a boolean value that indicates whether HybPiper should run intronerate or not.
* **-m, --heatmap_bool** is a boolean value that indicates whether HybPiper should create a gene_recovery heatmap (CURRENTLY UNIMPLEMENTED, was planned but never got implemented due to time constraints and other priorities).
* **-x, --timeout** is a numerical value that indicates an integer x, which allows HybPiper to kill jobs that take X percent longer than the average job, to prevent jobs getting stuck.
* **-n, --namelist** is a Boolean that indicates whether the script should write prefixes to a file called 'namelist.txt', (this is a fully functional feature needed for other features that were never implemented also due to time constraints).
